...
For a baseline social share functionality, providing all of the comma separated arguments to `metagen` shown in the [intro](/docs/intro) is recommended. If you want to add more meta tags that are not listed in the basic introduction example, see [parameters](/docs/category/options) for supported tags. You might only need a few `<meta>` tags instead of the whole set, simply provide the arguments you need and the ones not included won't be generated.
